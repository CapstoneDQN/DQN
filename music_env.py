...
class SongHistoryEnv(py_environment.PyEnvironment):
    ...
    def get_mf_recommendations(self, n=5):
        ...
        scores.sort(key=lambda x: x[1], reverse=True)
        recommendations = [s for s, _ in scores[:n]]
        
        #------------------디버그코드-------------------------------
        future_songs = set(self.user_sequence[self._step_count + 1:])
        overlap = set(recommendations).intersection(future_songs)
        
        print(f"\n=== 디버깅 정보 (Step {self._step_count}) ===")
        print(f"사용자 ID: {current_user_id}")
        print(f"현재까지 들은 곡 수: {len(current_history)}")
        print(f"미래에 들을 곡: {list(future_songs)[:10]}...")  # 처음 10개만
        print(f"추천곡: {recommendations}")
        print(f"겹치는 곡: {list(overlap)}")
        print(f"겹치는 곡 수: {len(overlap)}/{len(recommendations)}")
        print("=" * 50)
...
if __name__ == "__main__":
    ...
    for step_num in range(env._max_steps - 1):
        ...
        if not recommended_songs:
            ...
            print("⚠️ 추천곡 없음 - 랜덤 선택")
        else:
            # 추천 후보 중에서 랜덤으로 하나 선택
            chosen_song = recommended_songs[0]
        ...
        print(f"실제 다음 곡: {env.user_sequence[env._step_count]}")  # 실제 들은 곡
        print("액션 인덱스:", action)
